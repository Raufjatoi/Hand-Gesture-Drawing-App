...
# Setup drawing window size
drawing_window_size = (1200, 800)
paintWindow = np.ones((drawing_window_size[1], drawing_window_size[0], 3), dtype=np.uint8) * 255

# Function to draw interface buttons and selectable areas
def draw_interface():
    global paintWindow
    ...
    # Draw shape mode buttons
    cv2.rectangle(paintWindow, (700, 1), (770, 65), (0, 0, 0), 2)
    cv2.putText(paintWindow, "CIRCLE", (709, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)
    cv2.rectangle(paintWindow, (780, 1), (850, 65), (0, 0, 0), 2)
    cv2.putText(paintWindow, "RECTANGLE", (789, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)

    # Draw color selection blocks
    for i, color_name in enumerate(color_names):
        x0, y0, x1, y1 = 160 + i * 90, 1, 255 + i * 90, 65
        cv2.rectangle(paintWindow, (x0, y0), (x1, y1), colors[i], -1)
        cv2.putText(paintWindow, color_name.upper(), (x0 + 20, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)

    # Draw pen thickness options
    cv2.putText(paintWindow, "THICKNESS:", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)
    thickness_options = ["2px", "5px", "10px"]
    for i, thickness in enumerate(thickness_options):
        cv2.rectangle(paintWindow, (20, 120 + i * 40), (120, 160 + i * 40), (0, 0, 0), 2)
        cv2.putText(paintWindow, thickness, (50, 145 + i * 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)

    # Draw instruction for shortcuts
    instructions = [
        ("Press 'E' to toggle ERASER", (20, 240)),
        ("Press '1', '2', '3' to change PEN THICKNESS", (20, 280)),
        ("Press 'P' to PAUSE drawing", (20, 320)),
        ("Press 'S' to SAVE drawing", (20, 360)),
        ("Press 'Q' to QUIT", (20, 400))
    ]
    for text, pos in instructions:
        cv2.putText(paintWindow, text, pos, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)

# Main loop for capturing frames and processing gestures
while ret:
    ret, frame = cap.read()
    ...
    if result.multi_hand_landmarks:
        for handslms in result.multi_hand_landmarks:
            for lm in handslms.landmark:
                lmx = int(lm.x * drawing_window_size[0])
                lmy = int(lm.y * drawing_window_size[1])
